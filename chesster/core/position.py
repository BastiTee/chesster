...
class Position:
    ...
    def annotate(self, next_position):
    ...
    def _extract_information(self):
        ...
        if self.engine_info is None:
            return
        # get position score 
        self.score_original = self._get_engine_info_or_none('score[ ]+(cp|mate)[ ]+-?\d+[ ]?(lowerbound|upperbound)?')
        self._convert_original_score()
        # get best line 
        self.best_line = self._get_engine_info_or_none(' pv([ ]+[abcdefgh12345678]+)+')
    ...
    def _get_score_change(self, this_score, next_score):
        if 'M' in str(this_score) or 'M' in str(next_score):
            ...
            if 'M' in str(this_score) and not 'M' in str(next_score):
                return 'MM'
            return '{0}M'.format(int(re.sub('M', '', str(next_score))) - int(re.sub('M', '', str(this_score))))  
    ...
    def get_info_string(self):
        ...
        if not self.best_line:
            self.best_line = ''
        headers = [ 'm#', 'player', 'move', 'eval', 'diff', 'anno', 'comm', 'best_line_short' ]
        pos_data = [ self.move_number, player, self.move_played, self.score_display,
            self.score_change, self.annotation , self.comment,
            self.best_line[:10] ]
